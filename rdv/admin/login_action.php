...
if(isset($_POST["admin_email_address"]))
{
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE coach_email_address = :admin_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			{
				if($row["coach_status"] == 'Inactive')
				{
					$error = '<div class="alert alert-danger">Your account is Inactive, Contact Admin</div>';
				}
				else
				{
					if($_POST["admin_password"] == $row["coach_password"])
					{
						$_SESSION['admin_id'] = $row['coach_id'];
						$_SESSION['type'] = 'Coach';
						$url = $object->base_url . 'coach_schedule.php';
					}
					else
					{
