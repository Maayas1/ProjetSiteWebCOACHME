...
class Appointment
{
	public $base_url = 'http://localhost/rdv/admin/';
	public $connect;
	public $query;
	public $statement;
	public $now;
	...
	{
		$this->connect = new PDO("mysql:host=localhost;dbname=coach_appointment", "root", "");
	...
	function get_total_today_appointment()
	{
		...
		WHERE coach_schedule_date = CURDATE() 
		";
		$this->execute();
	...
	function get_total_yesterday_appointment()
	{
		...
		INNER JOIN coach_schedule_table 
		ON coach_schedule_table.coach_schedule_id = appointment_table.coach_schedule_id 
		WHERE coach_schedule_date = CURDATE() - 1
		";
		$this->execute();
		return $this->row_count();
	}
	...
	function get_total_seven_day_appointment()
	{
		...
		INNER JOIN coach_schedule_table 
		ON coach_schedule_table.coach_schedule_id = appointment_table.coach_schedule_id 
		WHERE coach_schedule_date >= DATE(NOW()) - INTERVAL 7 DAY
		";
		$this->execute();
		return $this->row_count();
	}
